...
import { ArrowLeft, Plus, Minus, Trash2, Clock, Check, X, Droplets, ChevronDown, ChevronUp, Video, Play, UploadCloud, Loader2, Zap } from "lucide-react";
...
export default function RecoveryPage() {
  ...
  // Video and customized inputs per card
  ...
  useEffect(()=>{
    setMounted(true);
    const s=localStorage.getItem("elite_pro_recovery_stack");
    if(s){setStack(JSON.parse(s));}
    else{
      const d=[{id:1,name:"Leg pressure machine"},{id:2,name:"Stretching"},{id:3,name:"Cold Shower"}];
      setStack(d);localStorage.setItem("elite_pro_recovery_stack",JSON.stringify(d));
    }
    const c=localStorage.getItem("elite_pro_recovery_completed");
    if(c) setCompleted(JSON.parse(c));
  },[]);

  const persistStack=(s)=>{setStack(s);localStorage.setItem("elite_pro_recovery_stack",JSON.stringify(s));};
  const persistCompleted=(c)=>{setCompleted(c);localStorage.setItem("elite_pro_recovery_completed",JSON.stringify(c));};

  const removeFromHistory = (actId) => {
    const act = stack.find(a => a.id === actId);
    if (!act) return null;
    
    const hist = JSON.parse(localStorage.getItem("elite_pro_recovery_history") || "[]");
    const idx = hist.findIndex(s => s.activities?.some(a => a.name === act.name));
    if (idx !== -1) {
      const copy = [...hist];
      const removed = copy.splice(idx, 1)[0];
      localStorage.setItem("elite_pro_recovery_history", JSON.stringify(copy));
      return removed;
    }
    return null;
  };

  const handleIncrement = (act) => {
    haptic.medium();
    setExpandedId(act.id);
    setShowFormFor(act.id);
  };

  const handleDecrement = (act) => {
    const id = act.id;
    ...
    if (count > 1) {
      ...
      const dur = logged[logged.length - 1] || 10;
      ...
      // Subtract from ring
      logTime("recovery", -dur, null, targetDate);
    } else if (count === 1) {
      haptic.heavy();
      setUnmarkTarget(act);
    }
  ...
  const handleVideoChange = async (e, actId) => {
    const file = e.target.files[0];
    if (!file) return;
    haptic.medium();
    setPendingVideo(p => ({ ...p, [actId]: file }));
    setPendingVideoUrl(p => ({ ...p, [actId]: URL.createObjectURL(file) }));
  };

  const addSessionInstance = async (actId, videoAttached = false) => {
    const act = stack.find(a => a.id === actId);
    if (!act) return;

    haptic.heavy();
    
    const dur = parseInt(selectedDuration[actId]);
    if (isNaN(dur) || dur <= 0) return;
    
    const chosenDateStr = selectedDate[actId] || todayDate;
    ...
    if (videoAttached && pendingVideo[actId]) {
      const file = pendingVideo[actId];
      ...
      try {
        ...
        const entry = {
          id: uid,
          type: "single",
          title: `${act.name} Video`,
          date: chosenDateStr,
          macroArea: "RECOVERY",
          subCategory: act.name,
          status: "NOT_ANALYZED"
        };
    ...
    // Log to ring
    logTime("recovery", dur, null, chosenDateStr);
    
    // Add to history
    const session = {
      id: newSessionId,
      date: chosenDateIso,
      activities: [{ name: act.name, duration: dur, videoAttached, videoUrl }],
      totalDuration: dur
    };
    const hist = JSON.parse(localStorage.getItem("elite_pro_recovery_history") || "[]");
    localStorage.setItem("elite_pro_recovery_history", JSON.stringify([session, ...hist]));
    
    // Reset inputs
    setPendingVideo(p => { const n = { ...p }; delete n[actId]; return n; });
    setPendingVideoUrl(p => { const n = { ...p }; delete n[actId]; return n; });
    setSelectedDuration(p => { const n = { ...p }; delete n[actId]; return n; });
    setExpandedId(null);
    setShowFormFor(null);
  };

  const completedCount = stack.filter(a => !!completed[a.id]?.count).length;
  const totalTime = stack.reduce((acc, act) => {
    const logged = completed[act.id]?.loggedDurations || [];
    const sum = logged.reduce((a, b) => a + b, 0);
    return acc + sum;
  }, 0);
  
  const progress = stack.length ? (completedCount / stack.length) * 100 : 0;
  ...
  return(
    <div className="app-container page-wrapper" style={{ minHeight: "100vh", paddingBottom: "100px", background: "linear-gradient(160deg, var(--background) 0%, #ffffff 50%, rgba(16, 185, 129, 0.12) 100%)", position: "relative", overflowX: "hidden" }} suppressHydrationWarning>
      
      {/* Decorative Blobs */}
      <div style={{ position: "absolute", top: "-5%", right: "-10%", width: "250px", height: "250px", background: "radial-gradient(circle, rgba(16,185,129,0.2) 0%, rgba(16,185,129,0) 70%)", borderRadius: "50%", zIndex: 0, pointerEvents: "none" }} />
      <div style={{ position: "absolute", bottom: "10%", left: "-10%", width: "200px", height: "200px", background: "radial-gradient(circle, rgba(16,185,129,0.15) 0%, rgba(16,185,129,0) 70%)", borderRadius: "50%", zIndex: 0, pointerEvents: "none" }} />
        ...
        {stack.length > 0 && (
          <div style={{ padding: "0 24px 20px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
              <span style={{ fontSize: "0.6rem", fontWeight: "800", color: "var(--gray-dim)", textTransform: "uppercase", letterSpacing: "0.08em" }}>Progress</span>
              <span style={{ fontSize: "0.6rem", fontWeight: "900", color: completedCount > 0 ? "#111" : "var(--gray-dim)" }}>
                {completedCount} / {stack.length} · {totalTime} min
              </span>
            </div>
            <div style={{ height: "6px", background: "var(--surface)", borderRadius: "3px", border: "1px solid var(--surface-light)", overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${stack.length?(completedCount/stack.length)*100:0}%`, background: ACCENT, borderRadius: "3px", transition: "width 0.6s cubic-bezier(0.34,1.56,0.64,1)" }}/>
            </div>
          </div>
        )}
      </header>
      ...
      <main className="v-stack" style={{ gap: "24px", padding: "24px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "12px", position: "relative", zIndex: 1 }}>
          <div className="interactive-card" style={{ padding: "16px", background: "#fff", border: "2px solid #111", borderRadius: "16px", display: "flex", flexDirection: "column", gap: "10px", boxShadow: "4px 4px 0px rgba(16, 185, 129, 0.4)" }}>
            <div style={{ width: "38px", height: "38px", borderRadius: "10px", background: "rgba(16, 185, 129, 0.15)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Droplets size={20} color={ACCENT} strokeWidth={2.5}/>
            </div>
            <div>
              <div style={{ fontSize: "1.8rem", fontWeight: "900", color: "#111", lineHeight: 1 }}>{stack.length}</div>
              <div style={{ fontSize: "0.65rem", fontWeight: "800", color: "var(--gray-dim)", textTransform: "uppercase", marginTop: "4px", letterSpacing: "0.05em" }}>Activities</div>
            </div>
          </div>
          
          <div className="interactive-card" style={{ padding: "16px", background: "#fff", border: "2px solid #111", borderRadius: "16px", display: "flex", flexDirection: "column", gap: "10px", boxShadow: "4px 4px 0px rgba(79, 70, 229, 0.3)" }}>
            <div style={{ width: "38px", height: "38px", borderRadius: "10px", background: "rgba(79, 70, 229, 0.15)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Clock size={20} color="#4F46E5" strokeWidth={2.5}/>
            </div>
            <div>
              <div style={{ fontSize: "1.8rem", fontWeight: "900", color: "#111", lineHeight: 1 }}>{totalTime}</div>
              <div style={{ fontSize: "0.65rem", fontWeight: "800", color: "var(--gray-dim)", textTransform: "uppercase", marginTop: "4px", letterSpacing: "0.05em" }}>Total Min</div>
            </div>
          </div>
        </div>
        ...
        <section style={{ position: "relative", zIndex: 1 }}>
          ...
          {stack.length === 0 && (
            <div style={{ 
              ...
              <div style={{ width: "56px", height: "56px", borderRadius: "12px", background: "#fff", border: "2px solid #eee", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Droplets size={24} color="#ccc" strokeWidth={2}/>
              </div>
              <div className="v-stack" style={{ gap: "4px" }}>
                <p style={{ fontWeight: "900", fontSize: "1rem", color: "#111", textTransform: "uppercase" }}>No activities yet</p>
                <p style={{ fontSize: "0.75rem", fontWeight: "700", maxWidth: "200px", margin: "0 auto" }}>Tap <span style={{ color: "#111", fontWeight: "900" }}>ADD</span> to build your stack.</p>
              </div>
            </div>
          )}
          ...
          <div className="v-stack" style={{ gap: "12px" }}>
            {stack.map((act, idx) => {
              const done = !!completed[act.id]?.count;
              const count = completed[act.id]?.count || 0;
              const open = expandedId === act.id;
              const fileObj = pendingVideo[act.id];
              const currentDur = selectedDuration[act.id] !== undefined ? selectedDuration[act.id] : "";
              ...
              return (
                ...
                >
                  {/* Card Header */}
                  <div 
                    onClick={() => { haptic.light(); setExpandedId(open ? null : act.id); }} 
                    style={{ padding: "16px", display: "flex", alignItems: "center", gap: "16px", cursor: "pointer" }}
                  >
                    {/* Status Icon */}
                    <div style={{ 
                      width: "48px", height: "48px", borderRadius: "50%", 
                      background: done ? ACCENT : "rgba(16, 185, 129, 0.1)", 
                      border: done ? `2px solid ${ACCENT}` : "2px solid rgba(16, 185, 129, 0.3)", 
                      display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
                      boxShadow: done ? `0 4px 12px rgba(16, 185, 129, 0.3)` : "none",
                      transition: "all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)"
                    }}>
                      {done ? <CompletionBadge isComplete={true} showText={false} color="#111" /> : <Zap size={22} color={ACCENT} />}
                    </div>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {done && (
                        <div style={{ display: "flex", gap: "10px", marginTop: "6px", alignItems: "center" }}>
                          <span style={{ fontSize: "0.6rem", fontWeight: "900", textTransform: "uppercase", padding: "4px 8px", borderRadius: "6px", background: "#111", color: ACCENT, border: "1px solid #111" }}>
                            COMPLETED
                          </span>
                        </div>
                      )}
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      {done && (
                        <div onClick={e => e.stopPropagation()} style={{ display: "flex", alignItems: "center", gap: "8px", background: "#111", color: "#fff", borderRadius: "8px", padding: "6px 12px" }}>
                          <button onClick={() => handleDecrement(act)} style={{ background: "none", border: "none", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><Minus size={14} strokeWidth={3}/></button>
                          <span style={{ fontSize: "0.8rem", fontWeight: "900", color: ACCENT }}>x{count}</span>
                          <button onClick={() => handleIncrement(act)} style={{ background: "none", border: "none", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><Plus size={14} strokeWidth={3}/></button>
                        </div>
                      )}
                      
                      {!open && !done && (
                        <button 
                          onClick={e => { e.stopPropagation(); haptic.heavy(); persistStack(stack.filter(a=>a.id!==act.id)); }} 
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                          ...
                          <div>
                            ...
                            <input 
                              type="date" 
                              max={todayDate} 
                              value={selectedDate[act.id] || todayDate} 
                              onChange={e => setSelectedDate(prev => ({ ...prev, [act.id]: e.target.value }))} 
                          ...
                          <div>
                            ...
                            <div style={{display:"flex",alignItems:"center",gap:10,background:"#fff",border:"2px solid #111",borderRadius:"10px",padding:"10px 14px"}}>
                              ...
                              <input type="number" inputMode="numeric" placeholder="Minutes" value={currentDur} onChange={e=>{
                                const v = e.target.value;
                                const p = parseInt(v);
                                setSelectedDuration(prev=>({...prev, [act.id]: (!isNaN(p) && p < 0) ? "0" : v}));
                          ...
                          <div style={{ background: "#fff", border: "2px solid #111", borderRadius: "12px", padding: "20px" }}>
                            ...
                            <div 
                              onClick={() => document.getElementById(`video-in-${act.id}`).click()}
                            ...
                            >
                              ...
                              {fileObj && (
                                <button onClick={(e) => { 
                                  e.stopPropagation(); 
                                  haptic.light();
                                  setPendingVideo(p => { const n = { ...p }; delete n[act.id]; return n; });
                                  setPendingVideoUrl(p => { const n = { ...p }; delete n[act.id]; return n; });
                            ...
                            <input id={`video-in-${act.id}`} type="file" accept="video/*" style={{ display: "none" }} onChange={(e) => handleVideoChange(e, act.id)} onClick={(e) => { e.target.value = null; }} />

                            {pendingVideoUrl[act.id] && (
                              <div style={{borderRadius:"8px",overflow:"hidden",border:"2px solid #111",position:"relative",background:"#000",aspectRatio:"16/9",maxHeight:160,marginTop:16}}>
                                <video src={pendingVideoUrl[act.id]} controls playsInline style={{width:"100%",height:"100%",objectFit:"contain"}}/>
                              </div>
                            )}
                          </div>
                          ...
                          <div style={{display:"flex",gap:12,marginTop:8}}>
                            {(() => {
                              const cDate = selectedDate[act.id] !== undefined ? selectedDate[act.id] : todayDate;
                              const cDur = parseInt(selectedDuration[act.id]);
                              const isReady = cDate && !isNaN(cDur) && cDur > 0;
                              return (
                                <button 
                                  className="interactive-btn" 
                                  onClick={() => isReady && addSessionInstance(act.id, !!fileObj)} 
                                  style={{ 
                      ...
                      {done && showFormFor !== act.id && (
                        <div style={{display:"flex",flexDirection:"column",gap:12, marginTop: "8px"}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                            ...
                            <button className="interactive-btn" onClick={() => handleIncrement(act)} style={{display:"flex",alignItems:"center",gap:4,padding:"6px 12px",borderRadius:"8px",background:"#111",color:"#fff",fontSize:"0.7rem",fontWeight:900,textTransform:"uppercase",border:"none",cursor:"pointer"}}>
                              <Plus size={14} strokeWidth={3}/> Add More
                            </button>
                          </div>
                          
                          <div style={{display:"flex",flexDirection:"column",gap:8}}>
                            {(completed[act.id]?.loggedDurations || []).map((loggedDur, sIdx) => {
                              return (
                                <div key={sIdx} style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fff",border:"2px solid #111",borderRadius:"10px",padding:"12px 16px"}}>
                                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                                    ...
                                    <span style={{fontSize:"0.85rem",fontWeight:900,color:"#111"}}>
                                      Session {sIdx+1}: {loggedDur} min
                                    </span>
                                  </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",backdropFilter:"blur(4px)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{background:"#fff",width:"100%",maxWidth:380,borderRadius:"20px",padding:"24px",border:"2px solid #111",boxShadow:"0 20px 40px rgba(0,0,0,0.2)",animation:"slideUp 0.3s ease"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
              <div>
                <p style={{fontSize:"0.6rem",fontWeight:900,color:"var(--gray-dim)",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:2}}>Recovery</p>
                <h3 style={{fontSize:"1.4rem",fontWeight:900,textTransform:"uppercase",letterSpacing:"-0.03em",color:"#111"}}>New Activity</h3>
              </div>
            ...
            <input autoFocus placeholder="e.g. Cryotherapy, Ice Bath..." value={newName} onChange={e=>setNewName(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&newName.trim()){haptic.medium();persistStack([...stack,{id:Date.now(),name:newName.trim()}]);setNewName("");setShowAdd(false);}}} style={{width:"100%",padding:"14px 16px",borderRadius:"10px",border:"2px solid #111",fontSize:"0.9rem",fontWeight:700,outline:"none",background:"#fff",fontFamily:"inherit",marginBottom:16}}/>
            <button className="interactive-btn" onClick={()=>{if(!newName.trim())return;haptic.medium();persistStack([...stack,{id:Date.now(),name:newName.trim()}]);setNewName("");setShowAdd(false);}} style={{width:"100%",padding:"16px",background:"#111",color:"#fff",border:"none",borderRadius:"10px",fontSize:"0.85rem",fontWeight:900,textTransform:"uppercase",cursor:"pointer"}}>
              Add to Stack
            </button>
          </div>
        </div>
          ...
          <div style={{background:"#fff",width:"100%",maxWidth:360,borderRadius:"20px",padding:"24px",border:"2px solid #111",boxShadow:"0 20px 40px rgba(0,0,0,0.2)",animation:"slideUp 0.25s ease"}}>
            <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:24}}>
              ...
              <p style={{fontSize:"0.85rem",color:"var(--gray-dim)",fontWeight:700,lineHeight:1.5,marginTop:8}}>
                Are you sure you want to unmark <strong>{unmarkTarget.name}</strong>? This will remove the session and subtract the time from the ring.
              </p>
            </div>
